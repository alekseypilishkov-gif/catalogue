...
<template>
  <a ref="card" :href="href" class="catalogue-card main-card" :class="{ 'has-cursor-glow': isCursorInside }" :data-state="state" @pointerenter="enter" @pointermove="moveCursor" @pointerleave="leave">
...
.main-card__title { color: var(--card-title-color); font-size: clamp(24px, 1.667vw, 32px); font-weight: 400; line-height: 1.23; letter-spacing: .05em; text-transform: uppercase; }
.main-card__description { font-size: clamp(16px, 1.042vw, 20px); line-height: 1.4; letter-spacing: .05em; }
@media (max-width: 1199px) { .main-card { padding: 24px; } }
